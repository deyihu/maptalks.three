...
//   }
// } = {};
const sdfGeneratorCache = {}, textMetricsCache = {};
export function isCJK(char) {
  return char >= 0x4E00 && char <= 0x9FFF;
}

export function getDefaultCharacterSet() {
  const charSet = [];
...
function extractFontStack(fontStack) {
...
export function generateSDF(fontStack = '', char) {
  ...
  if (!sdfGenerator) {
    // 创建 SDF
    sdfGenerator = sdfGeneratorCache[fontStack] = new TinySDF(fontsize, buffer, radius, cutoff, fontFamily, fontWeight);
  }
