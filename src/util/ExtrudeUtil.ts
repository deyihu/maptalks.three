 ...
 * @param {*} layer
 */
export function getExtrudeGeometry(polygon: PolygonType, height: number, layer: ThreeLayer, center?): THREE.BufferGeometry {
...
export function getExtrudeGeometryParams(polygon: PolygonType, height: number, layer: ThreeLayer, center, altCache?: any): MergeAttributeType {
 ...
 * @param {*} center
 */
export function getPolygonPositions(polygon: PolygonType, layer: ThreeLayer, center, isArrayBuff = false): Array<Array<Array<Array<number>> | ArrayBufferLike>> {
    if (!polygon) {
        return null;
    }
    let datas = [];
    if (polygon instanceof maptalks.MultiPolygon) {
        ...
        if (!isGeoJSONMulti(polygon)) {
            const data = getSinglePolygonPositions(polygon as any, layer, center || cent, isArrayBuff);
            datas.push(data);
        } else {
            const fs = spliteGeoJSONMulti(polygon);
            for (let i = 0, len = fs.length; i < len; i++) {
                datas.push(getSinglePolygonPositions(fs[i] as any, layer, center || cent, isArrayBuff));
            }
        }
    }
    return datas;
...
export function getSinglePolygonPositions(polygon: SinglePolygonType, layer: ThreeLayer,
    center, isArrayBuff = false): Array<Array<Array<number>> | ArrayBufferLike> {
